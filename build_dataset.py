...
from pipeline.utils import AgeGenderHelper
import numpy as np
import progressbar
import pickle
import json
...
# initialize the helper class, then build the set of image path and class labels
print("[INFO] buidling paths and labels...")
agh = AgeGenderHelper(config)
(trainPaths, trainLabels) = agh.buildPathsAndLabels()

# since we have the total number of images, compute the number of images
# for validation and testing
numVal = int(len(trainPaths) * config.NUM_VAL_IMAGES)
numTest = int(len(trainPaths) * config.NUM_TEST_IMAGES)

# class labels are represented as strings so we need to encode them
...
for (dType, paths, labels, outputPath) in datasets:
    ...
    # loop over each of individual images + labels
    for (i, (path, label)) in enumerate(zip(paths, labels)):
        # if we are building the training dataset
        # then compute the mean of each channel in the image
        if dType == "train":
