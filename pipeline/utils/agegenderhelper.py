...
class AgeGenderHelper:
    ...
    def buildAgeBins(self):
        # initialize the list of age bins based on Adience dataset
        ageBins = [(0, 2), (4, 6), (8, 13), (15, 20), (25, 32), (38, 43),
            (48, 53), (60, np.inf)]
    ...
    def buildOneOffMappings(self, le):
        ...
        classes = sorted(le.classes_,
            key = lambda x: int(x.decode("utf-8").split("_")[0]))
        oneOff = {}

        # loop over the index and name of the sorted class labels
        for (i, name) in enumerate(classes):
            # determine the index of the current class label name in the label encoder list
            # then initialize the index of the previous and next age groups adjacent to the
            # current label
            ...
            # construct a tuple that consists of the current age bracket,
            # the previous age bracket, and the next age bracket
    ...
    def buildPathsAndLabels(self):
        ...
        for foldPath in foldPaths:
            # load the contents of the folds file, skipping the header
            ...
            for row in rows:
                ...
                # construct the path to the input image and build the class label
                ...
                # if the label is None, then the age does not fit into
                # age brackets, simply ignore the sample
                if label is None:
                    continue
